...
class ReportApp:
    ...
    def _build_ui(self) -> None:
        ...
        self.delete_all_button = ttk.Button(
            scraped_controls,
            text="Delete All",
            command=self._delete_all_scrape_outputs,
        )
        self.delete_all_button.pack(side=tk.RIGHT)
    ...
    def _delete_all_scrape_outputs(self) -> None:
        company = self.company_var.get()
        if not company:
            messagebox.showinfo("Delete Scraped Data", "Please choose a company before deleting scraped results.")
            return
        scrape_root = self.companies_dir / company / "openapiscrape"
        if not scrape_root.exists():
            messagebox.showinfo("Delete Scraped Data", "No scraped outputs found for this company.")
            return
        confirm = messagebox.askyesno(
            "Delete All Scraped Data",
            "Are you sure you want to delete all scraped outputs for this company?",
        )
        if not confirm:
            return
        errors: List[str] = []
        for child in scrape_root.iterdir():
            try:
                if child.is_dir():
                    shutil.rmtree(child)
                else:
                ...
                errors.append(f"Could not delete {child.name}: {exc}")
        try:
            scrape_root.rmdir()
        except OSError:
            # Directory may remain when files failed to delete; ignore to allow partial cleanup.
            pass
        self._refresh_scraped_tab()
        if errors:
            messagebox.showwarning("Delete Scraped Data", "\n".join(errors))
